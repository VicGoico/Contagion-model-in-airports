...
public class VentanaControl extends javax.swing.JPanel {
	...
	private void initComponents(HashMap<Integer, Nodo> nodos) {
		...
		for (Map.Entry<Integer, Nodo> entry : nodos.entrySet()) {
			...
			if(informacionCompleta.containsKey(entry.getValue().getInfo().getCountry())) {
				informacionCompleta.get(entry.getValue().getInfo().getCountry()).put(entry.getValue().getInfo().getName(), 
						entry.getValue());
			}
			else {
		...
		String[] paises = new String[informacionCompleta.size()];
		...
		System.out.println(aeropuertosOrdenados.size());
		...
		jComboBoxPaises.setModel(
				new javax.swing.DefaultComboBoxModel<>(paises));
	...
	protected void PaisesActionPerformed(java.awt.event.ActionEvent evt) {
		...
		TreeMap<String,Nodo> aeropuertosPaisSeleccionado = informacionCompleta.get(
				this.jComboBoxPaises.getSelectedItem());
		int i = 0;
